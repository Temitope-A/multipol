    ...
    def __mul__(self, other):
        ...
        try:
            ...
            if isinstance(other, Number):
                ...
                raise TypeError('{} Is not a polynomial or number'.format(other))
        else:
            ...
            for i in range(0, self.poldegree + 1):
                for j in range(0, other.poldegree + 1):
                    a = self.pol[i]
                    b = other.pol[j]
                    if (isinstance(a, Number) and isinstance(b, Number) and isinstance(pol_list[i + j], Number)):
                        pol_list[i + j] += a*b
                    else:
                        if isinstance(a, Number):
                            a = [a]
                        ...
                        x = MultiPol(a,d) * MultiPol(b,d) + MultiPol(pol_list[i + j],d)
                        pol_list[i + j] = x.pol
...
# Evaluation via Horner's method ##############################################
    ...
    def evaluate(self, x):
        ...
        if self.var == 0:
            ...
            if len(x) < self.var:
                raise IndexError('Variables and points of evaluation not compatible')
            else:
